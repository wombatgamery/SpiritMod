			...
			for (int tileX = _x - _halfSize.X; tileX <= _x + _halfSize.X; tileX++)
			{
				...
				for (int tileY = surface; tileY <= _y + _halfSize.Y; tileY++)
				{
					...
					if (valid)
					{
						...
						if (IsOneOf(tile, ignoreTiles, ignoreWalls) || Main.tileDungeon[tile.TileType] || Main.wallDungeon[tile.WallType]) // now accounts for any dungeon blocks or walls added by other mods
							continue;
